from typing import Optional, Union, Dict, List, Any, Tuple, Sequence
...
def test_basic_parameters_tool_parsing():
    ...
    tools = [
        {"type": "function", 
         "function": Function.from_callable(get_weather).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
...
def test_optional_parameters_tool_parsing():
    ...
    tools = [
        {"type": "function", 
         "function": Function.from_callable(get_forecast).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
...
def test_union_type_parameters_tool_parsing():
    def generate_weather_string(degrees: Union[int, float], city: str = "New York", condition: Optional[str] = None) -> str:
        """
...
def test_complex_nested_parameters_tool_parsing():
    ...
    response = model.invoke([
        Message(role="system", content="You are an agent"),
        Message(role="user", content="I'm planning a trip to Barcelona. I like Spanish food and beach activities. My budget is between $1000 and $2000.")
    ])
    assert response.function_calls is not None
    assert len(response.function_calls) > 0
...
def test_list_with_generics_tool_parsing():
    ...
    tools = [
        {"type": "function", "function": Function.from_callable(get_city_weather_forecast).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
        Message(role="system", content="You are an agent"),
        Message(role="user", content="What's the weather forecast for New York, London, and Tokyo for the next 5 days?")
    ])
    assert response.function_calls is not None
    assert len(response.function_calls) > 0
...
def test_tuple_with_fixed_types_tool_parsing():
    ...
    tools = [
        {"type": "function", "function": Function.from_callable(get_temperature_range).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
...
def test_sequence_with_optional_values_tool_parsing():
    def get_historical_temperatures(
        city: str, 
        dates: Sequence[str],
        include_humidity: Optional[List[bool]] = None
    ) -> str:
        """
    ...
    tools = [
        {"type": "function", "function": Function.from_callable(get_historical_temperatures).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
        Message(role="system", content="You are an agent"),
        Message(role="user", content="What were the temperatures in Chicago on June 15-17, 2023? Include humidity information.")
    ])
    assert response.function_calls is not None
    assert len(response.function_calls) > 0
...
def test_optional_sequence_tool_parsing():
    ...
    tools = [
        {"type": "function", "function": Function.from_callable(get_weather_alerts).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
...
def test_mixed_sequence_types_tool_parsing():
    ...
    tools = [
        {"type": "function", "function": Function.from_callable(get_trip_weather).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
        Message(role="system", content="You are an agent"),
        Message(role="user", content="I'm planning a trip to Paris and Rome from July 10 to July 20, 2024. I might also visit Florence and Venice. What's the weather outlook?")
    ])
    assert response.function_calls is not None
    assert len(response.function_calls) > 0
...
def test_nested_pydantic_model_with_dict_tool_parsing():
    ...
    def plan_vacation(travel_plan: TravelPlan) -> str:
        ...
        return (f"Vacation plan for {travelers} travelers to {city}, {country} for {days} days{budget_info}. "
                f"Staying at {hotel} in a {room} room. "
                f"Food preferences: {food_prefs}. Activity preferences: {activity_prefs}.")
    
    tools = [
        {"type": "function", "function": Function.from_callable(plan_vacation).to_dict()}
    ]
    model = Gemini()
    model.set_tools(tools)
    response = model.invoke([
        Message(role="system", content="You are a travel agent assistant"),
        Message(role="user", content="I want to plan a trip to Paris, France for me and my partner for 7 days. We'd like to stay at The Grand Hotel in a deluxe room. We enjoy French cuisine and wine tastings, and want to visit museums and take walking tours. Our budget is $5000.")
    ])
    assert response.function_calls is not None
    assert len(response.function_calls) > 0